...
export const fundamentalsQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'The critical rendering path is the sequence of steps the browser takes to convert HTML, CSS, and JavaScript into pixels on the screen. It includes parsing HTML into the DOM, fetching and parsing CSS into the CSSOM, combining them into a render tree, running layout to compute geometry, and painting pixels. Optimizing this path means reducing the number of critical resources and the bytes needed to render above-the-fold content. In a real app, deferring non-critical CSS and inlining only the styles needed for the hero section can shave hundreds of milliseconds off first paint on mobile networks.',
...
// 6. Composite — GPU layers`,
  },
  {
    ...
    answer: 'Render-blocking resources are CSS and JavaScript files that prevent the browser from painting content until they are downloaded and processed. CSS blocks rendering because the browser needs styles before it can build the render tree and paint. Synchronous JavaScript in the head blocks HTML parsing until the script is fetched, parsed, and executed. Eliminating or deferring these resources is one of the highest-impact first optimizations. In a real app, moving analytics scripts to the bottom with defer and splitting CSS into critical inline styles plus async-loaded remainder often improves First Contentful Paint dramatically.',
...
<script async src="/analytics.js"></script> <!-- runs whenever ready, no order guarantee -->`,
  },
  {
    ...
    answer: 'Tree shaking removes unused exports from a bundle at build time through static analysis of ES module import graphs. Code splitting divides the remaining code into separate chunks loaded on demand. They solve different problems: tree shaking reduces what ships in each chunk, while splitting controls when each chunk loads. Both require ES modules and a bundler that supports dead code elimination. In a real app, tree shaking might drop 40 unused utility functions from lodash-es, and code splitting then loads the admin panel chunk only when an admin navigates there.',
...
<img src="/footer-logo.png" loading="lazy" fetchpriority="low" alt="" />`,
  },
  {
    ...
    answer: 'Above-the-fold content is everything visible in the viewport before the user scrolls. Optimizing its delivery directly impacts First Contentful Paint and Largest Contentful Paint because users judge speed by what they see first. Strategies include inlining critical CSS, preloading hero images, server-rendering visible HTML, and deferring everything below the fold. The goal is to render meaningful content in the first network round trips. In a real app, an e-commerce product page should server-render the product title, price, and hero image so shoppers see content within one second even on 3G.',
