...
# DevTools: F12 → Lighthouse → Analyze page load`,
  },
  {
    ...
    answer: 'Lighthouse evaluates five categories, each scored 0–100: Performance, Accessibility, Best Practices, SEO, and Progressive Web App (PWA). Performance measures loading speed and interactivity using metrics like LCP, TBT, and CLS. Accessibility checks WCAG compliance items like color contrast and ARIA labels. Best Practices covers security (HTTPS), console errors, and deprecated APIs. SEO audits meta tags, crawlability, and mobile-friendliness.',
...
// Score = weighted sum of individual metric scores (0–100)`,
  },
  {
    ...
    answer: 'The biggest levers are metrics with the highest weights: Total Blocking Time and Largest Contentful Paint together account for 55% of the score. Reducing JavaScript execution time, eliminating long tasks, and optimizing the LCP element have the largest impact. CLS improvements help the remaining 25%. Server response time, render-blocking resources, and image optimization indirectly affect multiple metrics. Third-party scripts are a frequent hidden cause of poor TBT.',
...
// CPU slowdown: 4x (mobile-like processing)`,
  },
  {
    ...
    answer: 'PageSpeed Insights (PSI) wraps Lighthouse lab data with Chrome UX Report field data from real users. PSI shows both what Lighthouse measured in a controlled test and what actual Chrome users experienced over the past 28 days. Lighthouse alone only provides lab data from a single simulated run. PSI also adds origin-level and URL-level field summaries with distribution charts. Use PSI for production assessment and Lighthouse locally for debugging.',
