...
/>`,
  },
  {
    ...
    answer: 'Web fonts can block rendering and cause layout shift if not loaded carefully. Best practices include preloading critical fonts, using font-display: swap to show fallback text immediately, subsetting fonts to include only needed characters, and self-hosting to eliminate third-party DNS/TLS overhead. Variable fonts reduce the number of font files needed. Limit font families and weights — each file is a render-blocking resource.',
...
<link rel="prefetch" href="/dashboard-chunk.js" as="script" />`,
  },
  {
    ...
    answer: 'Tree shaking is a dead code elimination technique where the bundler analyzes ES module import/export graphs and removes exports that are never imported. It requires static import/export syntax — dynamic requires and CommonJS modules limit shaking effectiveness. Marking package.json with "sideEffects": false tells the bundler it can safely drop unused modules. Combined with code splitting, tree shaking ensures each chunk contains only the code it uses.',
...
// No queuing, HPACK compressed headers`,
  },
  {
    ...
    answer: 'Dynamic import() returns a Promise that resolves to a module, letting the browser fetch and parse JavaScript only when the code path is needed. Bundlers automatically create separate chunks for each dynamic import, enabling route-based and component-based code splitting. Unlike static imports which are hoisted and bundled together, dynamic imports are evaluated at runtime. Combine with React.lazy() and Suspense for component-level splitting.',
