...
navigator.sendBeacon('/metrics', JSON.stringify({
  ...
  },
  {
    ...
    answer: 'The web-vitals library by Google provides small, tree-shakeable functions (onLCP, onINP, onCLS, onFCP, onTTFB) that use PerformanceObserver under the hood to capture metrics accurately. Each function registers an observer, calculates the metric value according to the official spec, assigns a rating (good/needs-improvement/poor), and invokes your callback. The library handles edge cases like bfcache restores, visibility changes, and SPA navigations. It is the recommended way to measure Web Vitals in production. In a real app, importing onLCP and sending results to Google Analytics 4 gives you field CWV data that matches what Google uses for ranking.',
...
observer.observe({
  ...
  },
  {
    ...
    answer: 'A long task is any continuous stretch of work on the main thread lasting 50ms or more. During a long task, the browser cannot respond to user input, run event handlers, or paint frames — directly causing poor INP and TBT. The Long Tasks API reports these via PerformanceObserver with type longtask, including attribution to specific scripts when available. Breaking tasks under 50ms keeps the page responsive. In a real app, a 200ms synchronous JSON.parse on a 5MB API response creates one long task — streaming parse or Web Worker offloading reduces it to multiple sub-50ms chunks.',
    code: `const observer = new PerformanceObserver((list) => {
  for (const entry of list.getEntries()) {
...
sendMetric('search-duration', measure.duration);`,
  },
  {
    ...
    answer: 'Sentry Performance tracks transactions (page loads, navigations, API calls) and spans (individual operations within a transaction) to build distributed traces. It automatically instruments pageload and navigation transactions with Web Vitals, and lets you create custom transactions for user flows. Spans show exactly where time is spent — database queries, HTTP requests, render time. Performance data links to errors so you can see if slow spans correlate with crashes. In a real app, a Sentry trace showing the /checkout transaction with a 2s span on payment-api helps you identify the backend bottleneck causing checkout abandonment.',
...
resources.forEach((r) => {
  ...
  },
  {
    ...
    answer: 'Synthetic monitoring runs scripted tests from controlled environments at regular intervals — checking if your site is up and measuring performance from specific locations. Tools include Lighthouse CI, WebPageTest, Pingdom, and Datadog Synthetics. RUM captures data from real users with all their device and network variance. Synthetic is consistent and good for regression detection and uptime; RUM is representative and good for understanding actual user experience. Use both together. In a real app, synthetic tests catch a deploy that broke your staging API response time, while RUM shows whether the fix improved p75 LCP for real mobile users.',
