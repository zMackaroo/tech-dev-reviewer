...
// 5. Lazy-loading the LCP image (never do this!)`,
  },
  {
    ...
    answer: 'Start by identifying the LCP element with Lighthouse or DevTools, then optimize its delivery path. Reduce TTFB with CDN caching, server-side rendering, or edge functions. Preload the LCP image, use modern formats like WebP or AVIF, and serve appropriately sized responsive images. Eliminate render-blocking resources, inline critical CSS, and defer non-essential JavaScript. Never lazy-load the LCP element.',
...
onLCP(metric => sendToAnalytics(metric)); // field data`,
  },
  {
    ...
    answer: 'CrUX is a public dataset of real user performance metrics collected from opted-in Chrome users worldwide. It aggregates field data by origin and URL, reporting LCP, INP, CLS, and other metrics at various percentiles. CrUX powers the field data section in PageSpeed Insights and Google Search Console Core Web Vitals reports. Data is updated monthly and requires sufficient traffic volume for URL-level granularity.',
