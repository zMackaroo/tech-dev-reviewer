...
If-None-Match: "abc123"  →  304 Not Modified`,
  },
  {
    ...
    answer: 'An ETag is a unique identifier the server assigns to a resource version, often a hash of the content. When a cached copy expires, the browser sends If-None-Match with the stored ETag. If the resource is unchanged, the server responds with 304 Not Modified and no body, saving bandwidth. Last-Modified works similarly with If-Modified-Since headers. ETags are more precise because they detect content changes even when the modification timestamp is the same.',
    code: `// First request
...
const queryClient = new QueryClient({
  ...
  },
  {
    ...
    answer: 'React Query (TanStack Query) caches API responses in memory keyed by queryKey arrays. When a component mounts, React Query checks if cached data exists and whether it is stale based on staleTime. Fresh data returns instantly from cache; stale data returns immediately while refetching in the background. gcTime (formerly cacheTime) controls how long inactive data stays in memory before garbage collection. React Query deduplicates simultaneous requests with the same key.',
...
// t=500s: remount → full loading state, new fetch`,
  },
  {
    ...
    answer: 'Memory cache (RAM) is extremely fast but cleared when the browser closes or under memory pressure. Disk cache (HTTP cache on disk) persists across sessions but is slower to read. The browser automatically promotes frequently accessed resources to memory cache and demotes them under pressure. Service worker Cache API writes to disk, surviving browser restarts. Application-level in-memory caches like React Query or a JavaScript Map are fastest but lost on page reload.',
...
self.addEventListener('install', (event) => {
  ...
  },
  {
    ...
    answer: 'Set TTL based on how frequently content changes and how tolerant users are of stale data. Static assets with content hashes get the longest TTL (1 year, immutable). HTML pages get short or zero TTL since they reference hashed assets. API responses vary: product catalogs might cache for hours, user-specific data for seconds or not at all. Use cache tags for granular purging instead of relying solely on TTL. Monitor cache hit ratio — below 80% suggests misconfigured TTL or too-aggressive purging.',
