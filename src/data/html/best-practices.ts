...
export const bestPracticesQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'Valid HTML follows the specification so browsers parse predictably, accessibility tools interpret structure correctly, and CSS selectors match as expected. Invalid nesting or duplicate ids create inconsistent DOM repair behavior across browsers. Validators like the W3C Nu Html Checker catch mistakes during development before they become subtle production bugs. For example, a form input outside any form may not submit while looking fine visually. In a real app, CI runs HTML validation on templates and Storybook stories to maintain markup quality alongside ESLint for JavaScript.',
    code: `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
  ...
  },
  {
    ...
    answer: 'Progressive enhancement builds a functional baseline with semantic HTML that works without JavaScript, then layers CSS for presentation and JavaScript for advanced interactions. Forms submit, links navigate, and native controls work before any bundle loads. This improves resilience on slow networks, restrictive environments, and for users who disable scripts. For example, a search form works with a full page reload via GET before JavaScript adds instant autocomplete. In a real app, e-commerce checkout must complete with plain HTML form POST even when React hydration fails.',
...
<script src="/search-enhance.js" defer></script>`,
  },
  {
    ...
    answer: 'Separation of concerns keeps HTML for structure and content, CSS for presentation, and JavaScript for behavior — avoiding inline styles, presentation tables, and onclick handlers in markup when maintainable alternatives exist. Clean HTML is easier to restyle, test, and parse by assistive technology. External stylesheets and deferred scripts keep markup readable and cacheable. For example, use class hooks in HTML and define appearance in CSS instead of style attributes on every element. In a real app, component libraries encapsulate this split while generated HTML from CMS stays free of inline style bloat.',
...
<article class="post">
  ...
  },
  {
    ...
    answer: 'Links opening new tabs with target="_blank" should include rel="noopener noreferrer" to prevent the new page from accessing window.opener and to reduce referrer leakage. Never inject unsanitized user HTML into the DOM; use textContent for plain text and trusted sanitizers for rich HTML. Validate href values to block javascript: URLs in user-generated links. For example, target="_blank" rel="noopener noreferrer" on external docs links closes the tab-nabbing vulnerability. In a real app, markdown renderers and CMS output filters enforce safe link attributes automatically.',
    code: `<a
...
<!-- Never: <a href="javascript:alert(1)">click</a> -->`,
  },
  {
    ...
    answer: 'HTML head directives significantly affect load performance: defer and async on scripts, preload for critical assets, fetchpriority="high" on LCP images, and loading="lazy" on below-fold media. Place stylesheets in head and non-critical scripts before closing body or with defer. Avoid render-blocking resources without justification. For example, script defer preserves order while not blocking HTML parsing. In a real app, performance budgets document which fonts and hero images get link rel="preload" in the root layout template.',
    code: `<head>
