...
export const metaSeoQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'The title element sets the page title shown in browser tabs, bookmarks, search engine results, and social share previews when no OG title overrides it. Search engines weight title heavily for ranking and display it as the clickable headline in SERPs. Titles should be unique per page, descriptive, and roughly 50–60 characters to avoid truncation. For example, "Pricing — Acme SaaS" is clearer than "Acme" on every page. In a real app, frameworks like Next.js expose a title template in layout metadata so each route sets a specific title automatically.',
    code: `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
  ...
  },
  {
    ...
    answer: 'The meta description provides a summary of page content shown beneath the title in search results when Google chooses to use it as the snippet. It does not directly affect rankings but influences click-through rate with compelling, accurate copy around 150–160 characters. Each page should have a unique description matching its content. For example, a product page description highlights key features and pricing context. In a real app, CMS fields or SEO components generate meta descriptions per route, falling back to a trimmed excerpt from main content.',
    code: `<head>
  ...
  },
  {
    ...
    answer: 'Open Graph meta tags control how URLs appear when shared on social platforms like Facebook, LinkedIn, Slack, and iMessage. og:title, og:description, og:image, and og:url define the preview card title, text, thumbnail, and canonical URL. Without them, platforms guess from page content, often picking poor images or truncated text. For example, og:image with a 1200x630 asset ensures a crisp preview card for blog posts. In a real app, marketing pages set OG tags in the head alongside Twitter Card tags for consistent sharing across networks.',
    code: `<head>
...
<!-- https://acme.com/products/wireless-keyboard?ref=email -->`,
  ...
  },
  {
    ...
    answer: 'hreflang annotations tell search engines about language and regional variants of a page so the correct version appears in localized search results. Implement via link rel="alternate" hreflang="x" in the head, HTTP headers, or sitemap entries. Each page should reference all language variants including itself, plus x-default for the fallback locale. For example, hreflang="fr-CA" points Canadian French users to the French Canadian URL. In a real app, international SaaS sites with /en/, /de/, and /ja/ routes set hreflang on every localized page to avoid duplicate content penalties.',
    code: `<head>
  ...
  },
  {
    ...
    answer: 'Beyond description and robots, useful meta tags include theme-color for browser UI tinting, author for attribution, and referrer for controlling Referer header leakage on outbound links. The charset and viewport meta tags are essential for correct rendering rather than SEO directly. Link rel="sitemap" can hint sitemap location though submitting via Search Console is standard. For example, meta name="theme-color" content="#2563eb" colors the mobile browser chrome to match brand. In a real app, a centralized Head component injects charset, viewport, robots, OG, and theme-color consistently across routes.',
    code: `<head>
  <meta charset="UTF-8" />
