...
export const advancedQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'The details element creates a disclosure widget that shows or hides content, with summary providing the always-visible clickable label. The open attribute sets the initial expanded state without JavaScript. Native disclosure works with keyboard and screen readers out of the box.',
    code: `<details>
...
<script>
  ...
  },
  {
    ...
    answer: 'Slots are placeholders inside a web component shadow DOM where light DOM content from the host element gets projected. Named slots use the name attribute to map specific children; the default slot catches unassigned content. This enables reusable component shells with user-supplied inner markup.',
...
<script>
  ...
  },
  {
    ...
    answer: 'The contenteditable attribute makes an element editable by the user in the browser, behaving like a rich text area with innerHTML mutations on input. It accepts true, false, or plaintext-only to restrict formatting. Unlike textarea, it supports mixed formatting but is harder to control, sanitize, and make accessible.',
...
<p contenteditable="plaintext-only">Edit this plain text.</p>`,
  },
  {
    ...
    answer: 'The download attribute on an a element hints that the linked resource should be downloaded rather than navigated to, optionally with a suggested filename. It works for same-origin URLs and blob URLs reliably; cross-origin behavior depends on CORS headers. Without download, the browser may preview PDFs or images instead of saving.',
...
<a href="/assets/brand-kit.zip" download>
  ...
  },
  {
    ...
    answer: 'Link rel="preload" tells the browser to fetch a high-priority resource early because it will be needed soon — fonts, critical scripts, or hero images. rel="prefetch" fetches low-priority resources likely needed on a future navigation, warming the cache for the next page. rel="preconnect" opens early connections to origins. Use as="script", as="style", or as="font" with preload so the browser knows the resource type.',
    code: `<head>
