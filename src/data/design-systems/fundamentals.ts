...
function SiteHeader() {
  ...
  },
  {
    ...
    answer: 'Atoms are indivisible UI elements that cannot be broken down further without losing meaning, such as a Button, Icon, or Text component. Molecules group two or more atoms into a single reusable unit with a shared purpose, like a FormField combining a Label and Input. Organisms are distinct interface sections composed of molecules and atoms, such as a ProductCard or NavigationBar that users recognize as a complete chunk of UI. The naming is a mental model for composition, not a strict file structure.',
...
<Alert variant="error" title="Error">
  ...
  },
  {
    ...
    answer: 'Documentation explains not just what components exist but when and how to use them, including do-and-don\'t examples, accessibility notes, and code snippets. Good docs bridge the gap between designers in Figma and engineers in the codebase so both teams share the same vocabulary. Usage guidelines prevent misuse, such as explaining that destructive actions should use the danger variant rather than a red primary button. Living documentation tied to the actual component source stays accurate as the system evolves.',
...
<ThemeProvider theme={isAcme ? acmeTheme : betaTheme}>
  ...
  },
  {
    ...
    answer: 'Adoption metrics track how many product teams import design system components versus building custom UI, often measured by component usage analytics or bundle analysis. Efficiency metrics include reduced design-to-dev time, fewer UI-related bugs, and faster feature delivery after adoption. Consistency audits compare live products against the system to find rogue colors, spacing, and one-off components. Developer satisfaction surveys reveal whether the system helps or hinders daily work.',
