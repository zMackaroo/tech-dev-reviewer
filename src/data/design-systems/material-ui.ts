...
export const materialUiQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'Material UI is a popular React component library implementing Google\'s Material Design with production-ready components like Button, TextField, Dialog, and DataGrid. It provides a comprehensive theming system, accessibility features, and extensive customization APIs. MUI ships both styled components (Material UI) and headless primitives (Base UI) for different use cases. The library handles complex patterns like focus management, keyboard navigation, and responsive layouts out of the box.',
...
const theme = createTheme({
  ...
  },
  {
    ...
    answer: 'ThemeProvider injects a theme object into the React tree via context so all descendant MUI components read shared design tokens. Wrap your app root with ThemeProvider passing the theme from createTheme. Nested ThemeProviders can override the theme for a subtree, useful for admin panels with a different color scheme. CssBaseline optionally normalizes browser defaults to match Material Design baselines.',
...
function App() {
  ...
  },
  {
    ...
    answer: 'The sx prop is a shorthand for applying theme-aware styles directly on MUI components and Box without writing separate CSS files. It accepts an object with CSS properties plus theme shortcuts like mt: 2 (margin-top from spacing scale), bgcolor: \'primary.main\', and responsive arrays. sx values resolve against the active theme so colors and spacing stay consistent. It supports pseudo-selectors and nested selectors for complex one-off styling.',
    code: `<Box
  sx={{
    p: 2,
...
>
  ...
  },
  {
    ...
    answer: 'The styled() function from @mui/material/styles creates styled components that access the theme in template literals or object syntax. It wraps the Emotion styled engine and supports both MUI components and native HTML elements. styled components automatically receive theme props and support variants via shouldForwardProp. This API suits reusable styled components that go beyond one-off sx prop usage.',
...
function SettingsForm() {
  ...
  },
  {
    ...
    answer: 'Material UI implements Google\'s Material Design with filled surfaces, elevation shadows, and ripple effects. Joy UI is MUI\'s newer library with a softer, more modern aesthetic using CSS variables, smaller bundle sizes, and a simpler API. Joy UI uses the sx prop and theming patterns familiar to MUI users but with different default styling and component design. Both share the same team and infrastructure but target different visual languages.',
    code: `// Material UI
...
<Box sx={{ p: { xs: 1, sm: 2, md: 4 } }} />`,
  },
  {
    ...
    answer: 'useMediaQuery is a React hook that listens to CSS media queries and returns a boolean indicating whether the query currently matches. MUI provides helpers like useMediaQuery(theme.breakpoints.up(\'md\')) to reactively respond to viewport changes. It supports SSR with defaultMatches and noSsr options to avoid hydration mismatches. Use it for conditional rendering when CSS-only responsive design is not enough.',
...
function Navigation() {
  ...
  },
  {
    ...
    answer: 'The theme typography object defines variants like h1 through h6, subtitle1, body1, body2, button, caption, and overline with fontSize, fontWeight, lineHeight, and letterSpacing. Typography component renders text with the correct variant and supports gutterBottom, noWrap, and color props. You customize the font family globally and override individual variant scales in createTheme. Responsive font sizes can adjust headings at different breakpoints via theme.typography.h1.',
...
<div className="grid grid-cols-12 gap-4 p-6">
  ...
  },
  {
    ...
    answer: 'Slot props let you pass props to internal sub-components (slots) of a complex MUI component without restructuring its API. This replaces the older componentsProps API with a clearer naming convention. Slots map to identifiable parts of a component like root, input, label, or listbox.',
    code: `<TextField
  label="Password"
  type="password"
  slotProps={{
...
<ThemeProvider theme={theme}>
  ...
  },
  {
    ...
    answer: 'Base UI (@mui/base) provides unstyled, accessible component primitives with behavior and ARIA attributes but no default CSS. You bring your own styling via CSS modules, Tailwind, or styled components for complete visual control. Base UI suits design systems that need MUI\'s robust accessibility and interaction logic without Material Design aesthetics. Components include Button, Input, Select, Modal, and Tabs as headless building blocks.',
