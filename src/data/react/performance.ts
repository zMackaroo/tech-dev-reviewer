...
// Skips re-render if data reference unchanged`,
  ...
  },
  {
    ...
    answer: 'Common causes include creating new object, array, or function references inline on every render and passing them as props, which breaks memoization in child components. Context value changes propagate re-renders to all consumers, and by default every parent re-render also re-renders all children even if their props are unchanged. Missing memo, useMemo, or useCallback in hot paths can amplify these issues, but the first step should always be profiling rather than blindly adding optimizations. In a real app, writing <Child style={{ color: "red" }} /> creates a new object each render and forces Child to update every time — hoisting the style or wrapping it in useMemo fixes the unnecessary work.',
...
function App() {
  ...
  },
  {
    ...
    answer: 'The React Profiler, available in React DevTools and programmatically via the Profiler component, records how long components take to render and why they re-rendered. It produces flame charts and ranked lists that highlight expensive components and unnecessary update cascades. This matters because guessing where performance problems live often leads to wasted effort optimizing components that are already fast. In a real app, profiling a sluggish product listing page might reveal that a parent re-render triggers hundreds of row components to update — pointing you toward memoization or virtualization rather than micro-optimizing unrelated code.',
  ...
  },
  {
    ...
    answer: 'Windowing (or virtualization) renders only the visible subset of a large list instead of mounting thousands of DOM nodes at once, dramatically reducing memory usage and layout cost. Libraries like react-window, react-virtualized, and TanStack Virtual calculate which items fall within the viewport and render just those rows with absolute positioning. This is essential for data-heavy UIs like long tables, chat histories, and infinite feeds where rendering every item would freeze the browser. In a real app, a 10,000-row transaction log displayed with react-window might render only ~15 visible rows at a time, keeping scroll performance smooth on modest hardware.',
