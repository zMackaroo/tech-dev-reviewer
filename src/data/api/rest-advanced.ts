...
await fetch('/users/42', {
  ...
  },
  {
    ...
    answer: 'On 429 Too Many Requests, read the Retry-After header (seconds or HTTP-date) and delay before retrying instead of immediately hammering the API. Implement exponential backoff with jitter for repeated 429s and reduce request frequency proactively using X-RateLimit-Remaining when exposed. Queue non-urgent writes during throttle periods.',
  ...
  },
  {
    ...
    answer: 'Hypermedia as the Engine of Application State (HATEOAS) means API responses include links (rel, href) describing available next actions so clients discover workflows dynamically instead of hard-coding URLs. It decouples clients from URL structure changes when relations stay stable. Maturity level 3 REST embraces hypermedia; most practical APIs use partial HATEOAS in pagination links.',
    code: `{
  "id": 9,
  "status": "pending",
  "_links": {
...
paths:
  ...
  },
  {
    ...
    answer: 'Batch endpoints accept an array of operations or ids in one POST to reduce round trips—POST /batch with [{ method, path, body }, ...] or POST /users/batch { ids: [...] }. Servers may process atomically or return per-item success/failure arrays. Respect payload size limits and rate limits when batching thousands of items.',
...
const { succeeded, failed } = await res.json();`,
  ...
  },
  {
    ...
    answer: 'Clients send a unique Idempotency-Key header on POST requests; the server stores the key with the first successful response and returns the same response for duplicate keys within a TTL window. This makes POST safe to retry on network failures without creating duplicate orders or charges. Keys should be UUIDs generated once per user action, not per retry attempt of the same action.',
...
export async function GET() {
  ...
  },
  {
    ...
    answer: 'Cursor pagination uses an opaque token (often encoding sort key + id) to fetch the next page—GET /feed?cursor=eyJpZCI6MTIzfQ&limit=20. It stays stable when rows are inserted or deleted during paging, unlike offset which skips or duplicates items on live feeds. Cursors require indexed sort columns and cannot jump to arbitrary page numbers easily.',
...
const next = await fetch(\`/api/feed?limit=20&cursor=\${meta.nextCursor}\`);`,
  },
  {
    ...
    answer: 'Return 207 Multi-Status or 200 with a results array detailing per-item status codes and errors instead of failing the entire batch on one bad row. Clients retry only failed items using returned identifiers. Document whether the batch is transactional (all-or-nothing) or best-effort partial apply.',
    code: `// 207 or 200 with mixed results
{
  ...
  },
  {
    ...
    answer: 'An API gateway sits as the single public entry point handling TLS termination, authentication, rate limiting, routing to internal services, request transformation, and response aggregation. It hides internal hostnames and lets you swap services without client changes. Downsides include an extra hop and potential bottleneck if not scaled.',
...
// Adds: X-User-Id, X-Request-Id from validated JWT`,
  },
  {
    ...
    answer: 'Liveness (/health) returns 200 if the process is running—used by orchestrators to restart crashed containers. Readiness (/ready) checks dependencies like database and cache and returns 503 when not ready to receive traffic during startup or maintenance. Keep health endpoints unauthenticated but rate-limited and lightweight.',
  ...
  },
  {
    ...
    answer: 'Clients send Accept-Encoding: gzip, br (Brotli) and servers compress large JSON responses to reduce bandwidth and latency. Small payloads may not benefit because compression overhead exceeds savings. Ensure proxies pass encoding headers correctly and decompress before parsing in tests.',
...
const job = await status.json(); // { status: 'processing' | 'complete', downloadUrl?: '...' }`,
  },
  {
    ...
    answer: 'Use tools like MSW (Mock Service Worker) to intercept fetch in the browser and return fixture JSON, run json-server or Prism against OpenAPI specs, or point to a dedicated staging API. Mocks should mirror real status codes, latency, and error shapes so integration surprises are minimal before connecting to production backends.',
