...
export const remoteQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'git remote add <name> <url> registers a new remote repository under a short alias, typically origin for the primary upstream. After adding, you can push, pull, and fetch using the alias instead of typing the full URL every time. Verify with git remote -v which shows fetch and push URLs. You can add multiple remotes—for example origin for your fork and upstream for the original repo. In a real app, after forking on GitHub, git remote add upstream https://github.com/org/original.git lets you pull mainline changes from the source project.',
...
git fetch --prune`,
  },
  {
    ...
    answer: 'git push -u origin <branch> pushes your local branch to the remote and sets the upstream tracking relationship so future git push and git pull commands work without specifying remote and branch names. Git records that your local feature/login tracks origin/feature/login. Use -u on the first push of a new branch; subsequent pushes need only git push. git branch -vv confirms upstream configuration and ahead/behind status. In a real app, after creating feature/checkout locally, git push -u origin feature/checkout publishes it and links tracking for the rest of the PR lifecycle.',
...
git push origin main`,
  },
  {
    ...
    answer: 'git remote prune <remote> removes stale remote-tracking branch references locally when those branches were deleted on the remote server. Without pruning, git branch -r still lists origin/feature/old long after someone deleted it on GitHub. git fetch --prune (or git fetch -p) fetches and prunes in one step—most teams use this by default. Pruning does not delete local branches, only outdated remote-tracking refs. In a real app, after a sprint ends and remote feature branches are cleaned up, git fetch --prune tidies your local branch -r output.',
...
git push --follow-tags`,
  },
  {
    ...
    answer: 'git push --force-with-lease updates the remote branch only if no one else has pushed new commits since your last fetch—preventing you from overwriting teammates work. Plain --force overwrites the remote branch unconditionally, which can delete others commits from the shared branch history. Use force-with-lease after amending or rebasing commits you already pushed on your own feature branch. Never force-push main without team policy. In a real app, after rebasing feature/api, git push --force-with-lease updates the PR safely.',
