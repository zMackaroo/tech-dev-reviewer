...
git restore --source=HEAD~2 src/app.ts`,
  },
  {
    ...
    answer: 'git clean removes untracked files from the working directory—files Git has never been told to track. -n performs a dry run showing what would be deleted without actually deleting. -f forces deletion; -d also removes untracked directories. -x removes ignored files too (like node_modules and build output), which is usually undesirable. Never run git clean -fdx without -n first on a project with local-only config. In a real app, git clean -fd clears leftover generated files after switching branches, but check -n output first.',
...
git clean -fdx`,
  },
  {
    ...
    answer: 'git commit --amend replaces the tip commit with a new one that includes any currently staged changes and/or an updated message. It rewrites the most recent commit hash, so never amend commits already pushed to a shared branch unless the team agrees and you force-push carefully. Use it to fix a typo in the commit message or add a file you forgot to stage. In a real app, after committing, you realize you left out package-lock.json—stage it and run git commit --amend --no-edit to add it without changing the message.',
...
git reset HEAD .env`,
  },
  {
    ...
    answer: 'git reset HEAD~1 moves the current branch back one commit using the default --mixed mode. The undone commit changes become unstaged modifications in your working directory—you have not lost the edits, only the commit record locally. This is useful for splitting an oversized commit or fixing a premature commit before pushing. If you already pushed, prefer git revert instead. In a real app, you committed too early with WIP code—reset HEAD~1, stage only the finished parts, and recommit cleanly.',
...
git reset --soft HEAD~1`,
  },
  {
    ...
    answer: 'git reflog records every movement of HEAD and branch tips, including commits removed by reset or rebase. Find the lost commit hash in the reflog, then git branch recovery <hash> or git reset --hard <hash> to restore it. Reflog entries expire after the default 90 days but usually suffice for immediate mistakes. This is your safety net when --hard reset goes wrong. In a real app, after git reset --hard HEAD~5 by mistake, git reflog shows the previous HEAD—reset --hard to that hash to recover.',
...
git restore --source=feature/old-ui -- .`,
  },
  {
    ...
    answer: 'git restore --staged . unstages every file in the staging area without modifying working directory content. Alternatively, git reset (with no commit argument) unstages all files using the default mixed reset against HEAD. Both leave your edits in place—only the index is cleared. Use this when git add . grabbed files you did not intend to commit yet. In a real app, before a careful partial commit, run git restore --staged . to start fresh, then git add -p to stage selectively.',
...
# git reset --hard HEAD~1  # only safe if not pushed`,
  },
  {
    ...
    answer: 'To throw away all local commits and uncommitted changes on a branch and match the remote, fetch first then reset hard to the remote tracking branch: git fetch origin && git reset --hard origin/main. This is destructive and cannot be undone except via reflog. For uncommitted changes only, git restore . and git clean -fd suffice without touching commits. In a real app, when your local main diverged badly from origin during a bad rebase experiment, reset --hard origin/main gives a clean slate.',
