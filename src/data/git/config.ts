...
git config --list --show-origin`,
  },
  {
    ...
    answer: 'Git embeds user.name and user.email in every commit you create as author metadata. Set them with git config --global user.name and user.email before your first commit on a new machine. Use --local to override for work vs personal repos with different identities. git log and GitHub attribution depend on correct values.',
...
git config user.email`,
  },
  {
    ...
    answer: 'git config alias.<name> "<command>" creates a shortcut expanding to any Git command or shell snippet. Aliases run with git <name>—for example alias.st = status lets you type git st. Use ! prefix for shell commands like !git checkout. Aliases save typing for frequent operations and can encode team conventions.',
    code: `# Simple alias
