...
git push origin --delete old-name`,
  },
  {
    ...
    answer: 'After git fetch, remote branches appear as remote-tracking refs like origin/feature/api. Use git switch feature/api or git checkout feature/api and Git auto-creates a local tracking branch if the name is unique among remotes. Explicitly: git switch -c feature/api origin/feature/api creates a local branch tracking the remote. Without fetch, your local repo may not know the branch exists.',
...
git branch -r`,
  },
  {
    ...
    answer: 'git merge --abort cancels an in-progress merge and restores your branch to the state before you ran git merge, as long as you have not yet committed the merge. It removes conflict markers and unstages merge-related changes. Use it when a merge went wrong, you chose the wrong target branch, or conflicts are too complex to resolve right now. This only works during an active merge; completed merges require git reset or git revert instead.',
    code: `# Start a merge
...
git checkout -B feature/rebuild origin/main`,
  },
  {
    ...
    answer: 'git branch --contains <commit> lists local branches whose history includes the given commit, meaning the commit is an ancestor of those branch tips. Add --merged or --no-merged to filter relative to your current branch. Use git branch -a --contains to include remote-tracking branches. This helps trace where a fix landed and whether it reached production branches.',
