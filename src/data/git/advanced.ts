...
export const advancedQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'git tag <name> creates a lightweight tag pointing at HEAD; git tag -a <name> -m "msg" creates an annotated tag with metadata, preferred for releases. Tags are not automatically pushed—use git push origin <tag> or git push --tags. List tags with git tag -l "v*". Tags mark specific points in history like v1.0.0 for deployments.',
...
git push origin v2.3.0`,
  },
  {
    ...
    answer: 'git bisect performs a binary search through commit history to find which commit introduced a bug. Start with git bisect start, mark the current bad commit with git bisect bad, and mark a known good commit with git bisect good <hash>. Git checks out the middle commit; you test and mark good or bad until the culprit is isolated. git bisect reset returns to your original branch.',
...
git bisect reset`,
  },
  {
    ...
    answer: 'git reflog records every position HEAD and branch refs have pointed to, including resets, rebases, and amended commits—even "deleted" history. Each entry has a timestamp and operation description. Use it to recover from mistakes like accidental git reset --hard or a bad rebase. Entries expire after gc.reflogExpire (default 90 days).',
...
git fsck --lost-found`,
  },
  {
    ...
    answer: 'git sparse-checkout limits your working directory to a subset of files in a large repository (monorepos), so you only checkout packages you work on. Enable with git sparse-checkout init --cone and set paths with git sparse-checkout set <dirs>. Requires partial clone or filter for best performance on huge repos. Git 2.25+ integrates sparse-checkout into the main workflow.',
