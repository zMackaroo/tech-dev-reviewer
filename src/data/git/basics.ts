...
git status --ignore-submodules`,
  },
  {
    ...
    answer: 'git add moves changes from your working directory into the staging area (index), marking them for inclusion in the next commit. You can stage entire files, specific paths, or interactively choose hunks with git add -p. Staging lets you split unrelated edits into separate, focused commits. Use git add . to stage all changes in the current directory tree, but prefer naming files explicitly when a commit should only cover part of your work. For example, stage only the API fix files while leaving WIP UI changes unstaged.',
...
git pull --rebase origin main`,
  },
  {
    ...
    answer: 'git log lists commits reachable from your current HEAD, newest first, showing hash, author, date, and message for each. Without flags the output is verbose, so most developers use formatting options daily. --oneline compresses each commit to one line; --graph draws an ASCII branch diagram; -n limits how many commits appear. Combine flags to scan recent history quickly during code review or incident response. In a real app, git log --oneline -20 shows the last twenty commits on your feature branch before you squash them for merge.',
...
git diff --name-only`,
  },
  {
    ...
    answer: 'git show presents the metadata and full patch introduced by a specific commit, tag, or other object. By default it shows HEAD—the tip of your current branch. Pass a commit hash to inspect exactly what that commit changed, which is invaluable during code review and debugging regressions. It also works with tags and tree objects. Unlike git log which lists many commits, git show zooms into one changeset. In a real app, when production breaks after a deploy, git show abc1234 quickly reveals whether a suspect commit touched the failing module.',
...
git help -w commit`,
  },
  {
    ...
    answer: 'The core loop is: edit files, check git status to see what changed, stage intentional changes with git add, commit with a clear message, and push to share with the remote. Always status before add to avoid staging secrets or build artifacts; always diff --staged before commit to verify the snapshot. Pull or fetch before push if teammates may have merged to your branch. This cycle is entirely local until push—commits exist on your machine first. In a real app, a typical feature session might be status → add two files → commit → repeat three times → push once.',
