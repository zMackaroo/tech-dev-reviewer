...
export const stashingQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'git stash saves your uncommitted changes (staged and unstaged tracked modifications by default) to a stack and reverts your working directory to a clean state matching HEAD. Use it when you need to switch branches or pull changes but are not ready to commit your current work. Stashes are local only—they are never pushed to remotes. git stash push -m "message" adds a descriptive label.',
...
git stash apply stash@{2}`,
  },
  {
    ...
    answer: 'git stash list shows all stash entries as a stack, newest first, labeled stash@{0}, stash@{1}, etc., with branch name and message. git stash show displays a summary diff of the latest stash; add -p for the full patch. This helps you find the right stash when you have several saved WIP states. Stashes persist until explicitly dropped or popped.',
...
git stash show -p stash@{1}`,
  },
  {
    ...
    answer: 'git stash drop stash@{n} removes one specific stash entry without applying it. git stash clear deletes the entire stash stack—use with caution as this is irreversible. Drop stashes you no longer need to reduce clutter; stale stashes confuse future you during git stash list. If a pop or apply succeeded and you used apply instead of pop, manually drop the stash afterward.',
...
git stash branch feature/resume-work`,
  },
  {
    ...
    answer: 'git stash push -- <paths> stashes changes only in the listed files or directories, leaving other modifications in your working directory. Combine with -m for a message. This is useful when you want to temporarily clear one file conflicts while keeping the rest of your edits active. You can also use git stash -p for interactive hunk selection similar to git add -p.',
