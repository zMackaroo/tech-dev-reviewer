...
.hero {
  ...
  },
  {
    ...
    answer: 'Pseudo-classes are selectors that target elements in a specific state or position without requiring extra HTML classes. They use a single colon syntax like :hover, :focus, :nth-child(2), or :not(.disabled). Interactive pseudo-classes (:hover, :active, :focus-visible) style user interactions, while structural ones (:first-child, :nth-of-type) select elements by their position in the DOM. Pseudo-classes add to specificity at the class level (0,0,1,0).',
    code: `a {
  color: #2563eb;
