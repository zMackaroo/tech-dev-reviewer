...
/* .Button_container_a3f2b { display: inline-flex; ... } */`,
  },
  {
    ...
    answer: 'In a CSS Module file, every class selector is automatically scoped to that file only—the bundler rewrites .header into something like .Dashboard_header_x7k9m so it cannot match elements outside this module. Global styles are opt-in via :global(), not the default behavior. This means you write plain class names like .active or .error without inventing long BEM-style prefixes for every component. For example, a Modal.module.css file can use .overlay and .content freely, knowing those selectors will never leak into your Sidebar styles.',
...
.content {
  ...
  },
  {
    ...
    answer: 'You import the module as a default export and reference classes through the imported object, typically on the className prop. The bundler (Vite, Webpack, or Next.js) processes the .module.css file and gives you an object like { button: "Button_button_a1b2c" }. You can combine multiple classes with template literals or a utility like clsx. In a real app, a ProductCard component imports styles from ProductCard.module.css and applies styles.button to the add-to-cart element, keeping styles colocated with the component file.',
...
export function Button({ label, primary }) {
  ...
  },
  {
    ...
    answer: 'The composes property lets one class inherit all declarations from another class within the same module or from an external file, similar to mixins but at the class level. It keeps shared base styles DRY without duplicating property blocks across selectors. You can compose from a local class or import styles from another .module.css file using composes: otherClass from "./shared.module.css". For example, a .btnPrimary class might compose .btnBase to inherit padding and font-size, then add only its background color override.',
    code: `.btnBase {
  padding: 8px 16px;
  font-size: 14px;
  border-radius: 4px;
...
:global(.sr-only) {
  ...
  },
  {
    ...
    answer: 'Because CSS Modules handle scoping automatically, you can use short, semantic class names like .header, .active, or .error instead of verbose BEM blocks like .product-card__title--highlighted. Many teams name the file after the component (UserProfile.module.css) and use camelCase for multi-word classes (styles.navItem) since they are accessed as JavaScript object keys. Consistency matters more than the specific convention—pick camelCase or kebab-case and stick with it across the project. For example, a team might use .isLoading and .isDisabled as state modifiers, mirroring common React prop naming patterns.',
...
// \`;`,
  },
  {
    ...
    answer: 'Both Vite and Next.js support CSS Modules out of the box with no extra configuration—simply name your file with the .module.css extension and import it in your component. Vite uses its built-in CSS pipeline to scope classes during dev and production builds, while Next.js handles CSS Modules in both the App Router and Pages Router automatically. Next.js also supports .module.scss for SCSS Modules with the same import pattern. In a real app, a Next.js dashboard page imports layout styles from Dashboard.module.css and Vite-based component libraries use the identical pattern, making CSS Modules portable across modern tooling.',
