...
/* @media (max-width: 767px) { .sidebar { display: none; } } */`,
  },
  {
    ...
    answer: 'Container queries let you apply styles based on the size of a parent container rather than the viewport, solving the problem of reusable components that must adapt inside different layouts. You mark a parent with container-type: inline-size (or size) and optionally container-name, then write @container rules that respond to that element\'s width. This means a card component can switch from stacked to side-by-side layout when its column is wide enough, even on a desktop page where the card sits in a narrow sidebar. For example, @container (min-width: 400px) changes a card from vertical to horizontal layout. In a real app, a dashboard widget uses container queries so it reflows correctly whether it spans one or two grid columns, without global viewport breakpoints.',
    code: `.card-wrapper {
...
.lead {
  ...
  },
  {
    ...
    answer: 'Traditional vh (1% of viewport height) is calculated against the layout viewport, which on mobile browsers ignores the dynamic appearance and disappearance of the address bar, often causing elements sized to 100vh to overflow or hide behind browser chrome. svh (small viewport height) uses the smallest possible viewport — address bar visible — while lvh (large viewport height) uses the largest — address bar hidden. dvh (dynamic viewport height) updates in real time as the browser UI shows or hides, giving the most accurate full-screen sizing. For example, min-height: 100dvh on a hero section fills the visible screen without a scrollbar gap when the URL bar collapses. In a real app, a full-screen onboarding flow uses 100dvh instead of 100vh to avoid the "jump" and extra scroll on iOS Safari.',
...
.thumbnail img {
  ...
  },
  {
    ...
    answer: 'The HTML picture element lets the browser choose different image sources via source elements with media or type attributes, while CSS controls how the chosen image is displayed. The img inside picture is what CSS styles — picture itself is a wrapper that does not render a box, so you target picture img or give the img a class. Common patterns include art direction (cropped mobile vs. wide desktop images) combined with width: 100% in CSS for fluid display. CSS cannot select which source loads, but it ensures whichever image arrives fits the layout. For example, a hero picture might load a portrait crop on narrow screens and a landscape crop on wide screens, both styled with object-fit: cover and height: 50dvh. In a real app, picture sources swap WebP and JPEG while a shared .hero-image class applies border-radius and object-fit consistently.',
...
.icon-button--compact::after {
  ...
  },
  {
    ...
    answer: 'Modern responsive layouts rely on a few repeatable Flexbox and Grid patterns instead of dozens of breakpoint-specific rules. Flexbox patterns include flex-wrap with gap for tag clouds, flex-direction: column on mobile switching to row in a media query for toolbars, and margin-inline-start: auto to push items to the end. Grid patterns include repeat(auto-fill, minmax(250px, 1fr)) for auto-responsive card grids, sidebar layouts with grid-template-columns: 1fr that become 240px 1fr at wider breakpoints, and subgrid (where supported) for aligned nested content. For example, auto-fill minmax eliminates manual column-count media queries because the browser adds or removes columns automatically. In a real app, a blog index uses Grid auto-fill for posts and Flexbox wrap for category filters, needing only one or two breakpoints for the sidebar.',
