...
.table thead th {
  ...
  },
  {
    ...
    answer: 'z-index controls the paint order of positioned elements (position other than static) along the z-axis, but only within the same stacking context. A new stacking context is created by properties like position with z-index, opacity less than 1, transform, filter, isolation: isolate, and will-change. Elements inside a context cannot layer above elements outside it regardless of z-index value — a child with z-index: 9999 still sits below a sibling context\'s z-index: 1. For example, a dropdown with z-index: 50 appears above page content but below a modal at z-index: 100 if they share the same root context. In a real app, you define a z-index scale (--z-dropdown: 50, --z-modal: 100) and ensure modals create their own stacking context so nested tooltips layer correctly inside the dialog.',
    code: `.dropdown {
  position: relative;
  z-index: 50;
}
...
.toast {
  ...
  },
  {
    ...
    answer: 'overflow determines what happens when content exceeds an element\'s box in the horizontal or vertical direction. visible (default) lets content spill outside the box; hidden clips it; scroll always shows scrollbars; auto shows scrollbars only when needed. overflow-x and overflow-y control each axis independently. Overflow affects margin collapse and creates a block formatting context when set to something other than visible. For example, overflow: hidden on a card with border-radius clips child images to the rounded corners. In a real app, a chat message list uses overflow-y: auto with a fixed height so older messages scroll while the input bar stays pinned below.',
    code: `.card {
  border-radius: 12px;
...
.table-wrapper {
  ...
  },
  {
    ...
    answer: 'The aspect-ratio property sets a preferred width-to-height ratio for a box, such as 16 / 9 or 1, so the element maintains that proportion as its width changes. It replaces older padding-top percentage hacks used for responsive embeds and image containers. The property works on any element and pairs well with width: 100% and object-fit: cover on child images or videos. For example, aspect-ratio: 16 / 9 on a video wrapper keeps the correct shape before the iframe loads. In a real app, a product grid uses aspect-ratio: 1 on image containers so every thumbnail is a consistent square regardless of the source photo dimensions.',
    code: `.video-wrapper {
  width: 100%;
  aspect-ratio: 16 / 9;
}
