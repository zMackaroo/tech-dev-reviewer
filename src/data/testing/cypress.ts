...
export const cypressQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'Cypress is a JavaScript end-to-end testing framework that runs tests inside the browser alongside your application, giving direct access to the DOM and network layer. It supports E2E testing of full user flows, component testing of isolated React/Vue components, and API testing via cy.request(). Cypress is known for its interactive test runner, time-travel debugging, and automatic waiting.',
...
describe("Checkout flow", () => {
  ...
  },
  {
    ...
    answer: 'cy.visit navigates to a URL and waits for the page load event. cy.get queries the DOM using CSS selectors, data attributes, or aliases and retries until the element exists or times out. cy.contains finds elements by their text content, partial text, or regex. All Cypress commands are chainable and automatically retry, reducing flaky timing issues.',
...
// );`,
  },
  {
    ...
    answer: 'Fixtures are static JSON, text, or binary files stored in cypress/fixtures/ that provide reusable test data. cy.fixture("users.json") loads the file and can be aliased with .as() for use in intercepts or direct assertions. Fixtures keep test data out of test files and version-controlled alongside tests.',
...
cy.fixture("users").as("users");`,
  },
  {
    ...
    answer: 'cy.intercept registers a listener on HTTP requests matching a method, URL pattern, or route alias, letting you stub responses or spy on real traffic. You can return fixture data, custom status codes, or delays to simulate slow networks and error states. Combined with cy.wait("@alias"), intercepts make API-dependent tests deterministic.',
...
cy.get("[data-testid=product-card]").should("have.length", 3);`,
  },
  {
    ...
    answer: 'Custom commands extend Cypress with reusable actions like cy.login() or cy.dataCy() defined in cypress/support/commands.ts. They wrap common sequences to reduce duplication across specs and enforce consistent patterns. Register with Cypress.Commands.add and declare TypeScript types for autocomplete.',
...
describe("DatePicker", () => {
  ...
  },
  {
    ...
    answer: 'Prefer data-testid, data-cy, or role-based attributes over CSS classes and DOM hierarchy, which change frequently during styling refactors. Avoid selecting by implementation details like nth-child unless necessary. Stable selectors decouple tests from visual redesigns and reduce maintenance.',
...
cy.get("form > div:nth-child(3) > button").click();`,
  },
  {
    ...
    answer: 'Avoid logging in through the UI in every test — use cy.session() to cache authenticated state, programmatic login via cy.request(), or inject tokens into localStorage before cy.visit(). cy.session() replays cookies and storage across specs while validating the session is still valid.',
...
cy.contains("Welcome").should("be.visible");`,
  },
  {
    ...
    answer: 'The command log appears in the Cypress test runner and shows every command, assertion, and network request in chronological order with before/after DOM snapshots. Clicking a command reveals the application state at that moment, enabling time-travel debugging. This makes failures much easier to diagnose than plain stack traces.',
...
cy.request({
  ...
  },
  {
    ...
    answer: 'Cypress.env() reads values from cypress.config.ts env block, cypress.env.json, or CLI flags like --env apiUrl=https://staging.example.com. Never commit secrets — use CI environment variables injected at runtime. Different env files support dev, staging, and production target URLs.',
...
// npx cypress run --env API_URL=https://staging.example.com`,
  },
  {
    ...
    answer: 'Aliases label DOM elements, fixtures, intercepts, or stubs for later reference with cy.get("@alias") or cy.wait("@alias"). They make tests readable and let Cypress retry against the same subject. Intercept aliases are especially common for waiting on specific API calls before asserting UI updates.',
...
// wait.until(ExpectedConditions.elementToBeClickable(btn));`,
  },
  {
    ...
    answer: 'Both chain assertions in Cypress, but cy.should() starts an assertion chain while cy.and() continues it with an additional condition on the same subject. Under the hood they are aliases — .and() reads naturally when combining multiple checks. Assertions auto-retry until they pass or time out.',
...
// Or: npx cypress run --record --key \${CYPRESS_RECORD_KEY}`,
  },
  {
    ...
    answer: 'cy.within() scopes subsequent commands to a specific DOM element, preventing accidental interactions with similar elements elsewhere on the page. It is essential when multiple components share the same selectors or when testing list items individually. The scope resets after the callback completes.',
...
cy.contains("Success").should("be.visible");`,
  },
  {
    ...
    answer: 'cypress/support/e2e.ts (or index.js) loads before every spec and is the place for global configuration, custom commands, and third-party plugin imports. It is not meant for test cases themselves. Import commands.ts here and add global beforeEach hooks for common setup like preserving cookies.',
