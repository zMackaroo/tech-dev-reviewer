...
// Aim: most tests at the bottom, critical flows at the top`,
  },
  {
    ...
    answer: 'Unit tests verify a single function, class, or component in isolation, often with dependencies mocked or stubbed. Integration tests verify that multiple units work together correctly, such as a service calling a database or a React component interacting with a context provider. End-to-end (E2E) tests drive the full application through a browser or API client, simulating real user behavior from start to finish.',
...
// cy.visit("/products"); cy.get("[data-testid=add]").click();`,
  },
  {
    ...
    answer: 'Test-Driven Development is a workflow where you write a failing test first, then write the minimum code to make it pass, and finally refactor while keeping tests green. The cycle is often summarized as Red-Green-Refactor. TDD forces you to define expected behavior before implementation and produces a safety net that makes refactoring less risky.',
...
// 3. REFACTOR — improve without changing behavior`,
  },
  {
    ...
    answer: 'Test coverage measures what percentage of your source code is executed when the test suite runs, commonly reported as line, branch, or function coverage. High coverage indicates more code paths were exercised, but it does not guarantee tests are meaningful or that edge cases are handled. Teams typically set a minimum threshold in CI (e.g., 80%) as a floor, not a goal to chase blindly.',
    code: `// jest.config.js
module.exports = {
...
test("loads user profile", async () => {
  ...
  },
  {
    ...
    answer: 'AAA stands for Arrange, Act, and Assert — a structure that keeps tests readable and consistent. In the Arrange phase you set up inputs, mocks, and initial state. In the Act phase you invoke the function or trigger the behavior under test. In the Assert phase you verify the outcome matches expectations.',
...
expect(getDate()).toBe("2024-01-01");`,
  },
  {
    ...
    answer: 'A good unit test is fast, isolated, deterministic, and clearly named so failures are easy to diagnose. It tests one behavior per test case, avoids depending on external services or shared mutable state, and uses meaningful assertions rather than checking implementation details. Tests should survive refactors as long as behavior stays the same.',
...
test("calls filter internally", () => {
  ...
  },
  {
    ...
    answer: 'Regression testing re-runs existing tests after code changes to ensure previously working behavior has not broken. Automated test suites in CI are the primary regression safety net on every pull request and merge. Without it, fixes and new features can silently reintroduce old bugs.',
