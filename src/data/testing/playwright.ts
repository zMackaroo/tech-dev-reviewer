...
export const playwrightQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'Playwright is a Microsoft-maintained browser automation library that supports Chromium, Firefox, and WebKit with a unified API. It uses the browser\'s native automation protocols, runs tests in parallel out of the box, and includes auto-waiting, network interception, and tracing built in. Unlike Cypress, Playwright can test multiple tabs, iframes, and browser contexts in one test.',
...
await expect(page.getByRole("alert")).toHaveText("Saved successfully");`,
  },
  {
    ...
    answer: 'Configure projects in playwright.config.ts with different browser devices or channel settings for chromium, firefox, and webkit. Playwright Test runs each project, and you can tag tests to run only on specific browsers. CI matrices can shard projects across workers for speed.',
...
// Run one: npx playwright test --project=firefox`,
  },
  {
    ...
    answer: 'The Trace Viewer captures a detailed recording of test execution including DOM snapshots, network requests, console logs, and screenshots at each step. Enable tracing with trace: "on-first-retry" or trace: "retain-on-failure" in config, then open the trace with npx playwright show-trace trace.zip. It is invaluable for debugging CI failures you cannot reproduce locally.',
    code: `// playwright.config.ts
export default defineConfig({
  use: {
...
await expect(page.getByText("Welcome")).toBeVisible();`,
  },
  {
    ...
    answer: 'Playwright Test runs test files in parallel by default using worker processes, each with isolated browser contexts. Configure workers in playwright.config.ts or via --workers CLI flag. Tests within the same file run sequentially unless test.describe.configure({ mode: "parallel" }) is set. Parallel runs dramatically reduce CI time but require test isolation.',
    code: `// playwright.config.ts
export default defineConfig({
  fullyParallel: true,
...
// CLI: npx playwright test --workers=8`,
  },
  {
    ...
    answer: 'Use page.route() to intercept requests matching a URL pattern and fulfill them with mock responses, abort them, or modify real responses. route.fulfill() returns static JSON, while route.fetch() lets you modify a real upstream response. Combine with page.waitForResponse() to synchronize on mocked calls.',
...
await expect(page.getByText("Widget")).toBeVisible();`,
  },
  {
    ...
    answer: 'Fixtures are Playwright\'s dependency injection system — built-in fixtures like page, context, and browser are provided to each test, and you can define custom fixtures with test.extend(). Fixtures handle setup and teardown automatically and can depend on other fixtures. They replace global beforeEach hooks with composable, typed setup.',
...
test("dashboard loads", async ({ authenticatedPage }) => {
  ...
  },
  {
    ...
    answer: 'Start waiting for the download event before triggering the action that initiates it, then save or inspect the downloaded file. page.waitForEvent("download") returns a Download object with path(), suggestedFilename(), and saveAs() methods. This pattern handles browser download dialogs that cannot be interacted with directly.',
  ...
  },
  {
    ...
    answer: 'test.describe groups related tests under a shared label, supporting nested describes, beforeEach/afterEach hooks, and configuration like retries or timeouts at the group level. describe.configure controls parallel vs serial execution within a file. Organizing by feature or page keeps specs maintainable.',
  ...
  },
  {
    ...
    answer: 'Log in once in a setup project, save storageState to a JSON file, and reuse it across tests via storageState in config or test.use(). This avoids repeating login UI steps in every spec and speeds up suites significantly. storageState includes cookies and localStorage.',
...
// await page.getByRole("button").click(); // throws`,
  },
  {
    ...
    answer: 'Use page.setViewportSize() or device presets from playwright.devices to simulate mobile, tablet, and desktop dimensions. Playwright can also emulate touch, geolocation, and color scheme preferences. Run the same test across viewport projects in config for comprehensive coverage.',
...
test("search input renders", async ({ mount }) => {
  ...
  },
  {
    ...
    answer: 'Set retries, workers, forbidOnly, and reporter in playwright.config.ts with CI-specific conditionals. Install browsers in CI with npx playwright install --with-deps. Use blob reporter or HTML reporter for artifacts uploaded on failure. Sharding splits tests across multiple CI jobs with --shard=1/4.',
    code: `// playwright.config.ts
export default defineConfig({
