...
export const lmStudioQuestions: InterviewQuestion[] = [
  {
    ...
    answer:
      'LM Studio is a desktop application for discovering, downloading, and running local large language models on your machine with a chat UI and local OpenAI-compatible API server. It targets developers and power users who want offline inference, privacy, or cost-free experimentation without cloud API keys. Models run via llama.cpp backends with GPU acceleration when available. For example, you download Mistral or Llama locally and prototype RAG before moving to production cloud models. In a real app workflow, LM Studio serves as a dev sandbox while production uses hosted APIs or self-hosted vLLM.',
...
Full FP16 — best quality, highest VRAM`,
  },
  {
    ...
    answer:
      'VRAM determines max model size and context length — 8GB fits many 7B quants, 24GB+ enables larger models or longer contexts. CPU-only mode works but is dramatically slower for interactive apps. Monitor temperature and throttle batch size; close other GPU apps to avoid OOM crashes mid-inference. Context window settings increase memory linearly — do not set 128k context on hardware that cannot support it. For example, a Mac with 16GB unified memory might run 8B Q4 smoothly but struggle with 34B. In a real app dev loop, match local model size to your machine for acceptable iterate speed.',
    code: `Rough guide:
7B Q4  → ~4–6 GB VRAM
13B Q4 → ~8–10 GB VRAM
Context ↑ → RAM/VRAM ↑ linearly`,
  },
  {
    ...
    answer:
      'LM Studio offers a polished GUI, model browser, and chat testing with one-click server start — great for exploration. Ollama is CLI-first, popular in dev scripts and Docker, with simple `ollama run` ergonomics. Cloud APIs provide best quality, scaling, and no hardware ops at per-token cost. Choose LM Studio for interactive local dev; Ollama for automation; cloud for production scale. For example, prototype prompts in LM Studio chat, script benchmarks with Ollama, deploy with OpenAI. In a real app, many teams use all three in different lifecycle stages.',
...
OpenAI     → production scale + latest models`,
  },
  {
    ...
    answer:
      'LM Studio supports embedding models through the same local server when you load an embedding-capified GGUF model, enabling fully offline RAG pipelines with local vector stores like Chroma. Quality and speed depend on the embedding model chosen — smaller models are faster but may hurt retrieval recall versus OpenAI embeddings. Same chunking and eval practices apply as cloud RAG. For example, embed docs with nomic-embed locally, store in Chroma, query via local chat model — no data leaves the machine. In a real app, local RAG suits air-gapped or PII-sensitive prototypes before cloud deployment.',
...
LM Studio (chat model) + retrieved chunks → answer`,
  },
  {
    ...
    answer:
      'Local models lag frontier cloud models on reasoning, tool use reliability, and long-context performance; single-machine deployment does not scale horizontally or offer SLAs. No built-in auth, billing, or team key management — you build the production wrapper. Updates, monitoring, and failover are your responsibility unlike managed APIs. For example, complex agent workflows may fail more often locally and need simpler prompts. In a real app, use LM Studio for dev/offline; production typically needs vLLM, TGI, or cloud with load balancing and observability.',
    code: `Local limits:
...
• You own uptime, security, updates`,
  },
  {
    ...
    answer:
      'Adjust temperature and max tokens, try a higher quantization or different base model, and simplify prompts — local models often need shorter, more explicit instructions than GPT-4. Use the chat UI to iterate prompts before API integration; compare same prompt across models in Model Switcher. Check server logs for context truncation or OOM errors masquerading as gibberish. For example, if JSON mode fails, add few-shot examples or switch to a model fine-tuned for instruction following. In a real app, maintain a eval set of 20 representative prompts scored per local model before choosing one for offline demos.',
