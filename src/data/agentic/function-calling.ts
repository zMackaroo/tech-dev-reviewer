...
export const functionCallingQuestions: InterviewQuestion[] = [
  {
    ...
    answer:
      'Function calling lets the model return structured tool invocations — function name and JSON arguments — instead of only natural language, so your app executes real APIs, queries, or code and feeds results back to the model. The API exposes tool schemas describing parameters; the model chooses when and how to call them based on user intent. This is the foundation of agents, copilots, and grounded assistants. For example, get_weather(city="London") runs your weather API and the model summarizes the JSON response for the user. In a real app, you validate arguments with Zod or JSON Schema before execution to block malformed or malicious calls.',
    code: `tools=[{
  "type": "function",
  "function": {
  ...
  },
  {
    ...
    answer:
      'Tool descriptions are prompts — name should be verb_noun clear, description must say when to use and when NOT to use the tool, and parameter descriptions explain format and enums. Ambiguous descriptions cause wrong tool selection or invented arguments. Include examples in description for complex formats like date ranges or IDs. For example, "search_docs: Use for company policy questions. Do NOT use for live order status — use get_order instead." In a real app, review tool-call logs weekly and refine descriptions for frequently mis-selected tools.',
...
Do not use for refunds — use create_refund."`,
  },
  {
    ...
    answer:
      'Structured output forces the model response itself to match a JSON Schema — good for extraction and classification returned to your UI. Function calling triggers external side effects — database, email, search — with the model requesting execution through your host. They can combine: structured output for parsing user intent, function calling for actions. Choose structured output when no external tool is needed but format must be reliable. For example, extract {date, guests} via structured output then call book_restaurant via function calling. In a real app, avoid duplicating the same schema in both tools and response_format without reason.',
...
tools=[{"type": "function", ...}]`,
  },
  {
    ...
    answer:
      'Return tool messages describing errors clearly — HTTP status, retryable flag, user-safe message — so the model can apologize, retry, or escalate instead of hallucinating success. Set timeouts on every external call; partial failures in parallel batches should report per-tool status. Log errors with correlation IDs linking to user sessions for support. For example, tool returns {"error": "Order not found", "code": 404} and the model asks the user to verify their order ID. In a real app, circuit-break repeated failures to the same dependency and fall back to cached or human handoff.',
    code: `try:
  ...
  },
  {
    ...
    answer:
      'Parallel function calling lets the model emit multiple independent tool_calls in one assistant message; your host executes them concurrently and returns all tool results before the next completion. Safe when calls are read-only or touch disjoint resources — two searches, two lookups. Unsafe when order matters or calls conflict — debit then credit, sequential workflow steps. For example, fetching weather and timezone for a city in parallel is fine; creating two tickets with duplicate detection is not. In a real app, detect dependency in tool design and document sequential-only tools in descriptions.',
...
results = await asyncio.gather(*[run(c) for c in calls])`,
  },
  {
    ...
    answer:
      'LangChain bind_tools attaches schemas to ChatModel; ToolNode executes matching Python/JS functions when the model returns tool_calls. LangGraph routes agent → tools → agent with conditional edges for multi-step tool loops and checkpointing. The framework handles message formatting but you still own auth, validation, and idempotency inside each tool. For example, @tool decorator functions auto-generate schemas from type hints and docstrings. In a real app, keep business logic in service layers, not fat tool functions, for testability.',
...
llm_with_tools = llm.bind_tools([search_kb])`,
  },
  {
    ...
    answer:
      'Authenticate the user before executing tools on their behalf; pass user_id from session, never from model-generated arguments alone. Allowlist tools per role, sanitize arguments, and require confirmation for destructive operations regardless of model requests. Sandboxed code execution and SQL tools need read-only modes and query planners to block DROP or exfiltration. Prompt injection can trigger malicious tool args — validate against business rules server-side. For example, delete_account must verify session owner matches argument user_id. In a real app, security policy lives in code, not in prompts hoping the model behaves.',
