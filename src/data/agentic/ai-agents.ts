...
• Reflexion — critique and retry`,
  },
  {
    ...
    answer: 'Set hard limits: max_iterations, max_tool_calls, max_tokens, and wall-clock timeouts; stop when the model repeats the same failed action or returns without progress. Detect loops by hashing recent (action, args) pairs and breaking when duplicates appear. Require explicit finish tools or structured "done" signals instead of free-text stop.',
    code: `MAX_STEPS = 10
seen = set()
for step in range(MAX_STEPS):
  ...
  },
  {
    ...
    answer: 'Tool grounding means the agent\'s claims about the world come from tool observations — database rows, API responses, file contents — not from model parametric memory, reducing hallucination on factual tasks. Prompts should instruct the model to cite tool output and refuse when tools return empty or error states. Without grounding, agents confidently invent prices, ticket statuses, or code behavior.',
...
Never invent order IDs or prices."`,
  },
  {
    ...
    answer: 'Separate working memory (current task context, recent tool outputs) from long-term memory (user preferences, past sessions) stored in vector DB or structured stores. Summarize or compress old turns when approaching context limits, keeping key facts and open sub-goals. Scoped memory per user and per task prevents cross-contamination in multi-tenant systems.',
    code: `Memory layers:
...
• Episodic: structured task logs for audit`,
  },
  {
    ...
    answer: 'Agent evals test end-to-end task success — did the agent book the meeting, fix the bug, resolve the ticket — not just single-turn BLEU scores. Use trajectories: expected tool call sequences, final state checks, and LLM judges with rubrics for partial credit. Include adversarial cases: missing data, tool errors, and prompt injection attempts.',
