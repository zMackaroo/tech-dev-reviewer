...
console.log(rabbit.jumps); // true — own property`,
  ...
  },
  {
    ...
    answer: '__proto__ (or better, Object.getPrototypeOf) is the actual link on an instance pointing to its parent object in the chain. prototype is a property that exists only on constructor functions and becomes the __proto__ of objects created with new. When you call new Person("Bob"), the instance\'s prototype chain is wired to Person.prototype where greet is defined. For example, fixing a bug in Person.prototype.greet updates every Person instance at once because they all delegate to the same prototype object.',
...
console.log(car.drive());`,
  ...
  },
  {
    ...
    answer: 'ES6 classes are syntactic sugar over the same constructor-plus-prototype model—extends and super compile down to prototype chaining under the hood. They add clearer syntax for inheritance, static methods, and true private fields with # that plain prototype code could not enforce as strictly. You still get one shared prototype for methods rather than copying them per instance. In a real app, a React component class extending Component uses the same prototype machinery as function Foo() { ... }; Foo.prototype.render = ... did before hooks became common.',
    code: `class Animal {
...
console.log(new Dog("Rex").speak());`,
  ...
  },
  {
    ...
    answer: 'Object.create(proto) makes a new object whose prototype is set directly to the object you pass in, without running any constructor function. new Constructor() also sets up the prototype link but additionally executes the constructor to initialize instance state on this. Use Object.create when you only need delegation without constructor side effects. For example, Object.create(adminDefaults) is handy for cloning permission templates, while new AdminUser() runs setup logic like validating an employee ID.',
...
// console.log(s.#key); // SyntaxError`,
  ...
  },
  {
    ...
    answer: 'Inheritance models an "is-a" relationship—a Dog is an Animal and inherits its entire interface and state shape. Composition models "has-a" behavior by combining smaller objects or functions, like giving duck both canFly and canSwim without a deep class tree. Favoring composition avoids fragile base classes and diamond-shaped hierarchy problems. In a real app, instead of Server extends Database extends Logger, you might pass a logger and db client into a Server constructor so each piece can be swapped or tested independently.',
    code: `// Composition
...
console.log(duck.fly(), duck.swim());`,
  ...
  },
  {
    ...
    answer: 'Object.getPrototypeOf returns the object that another object delegates to—the standard, safe replacement for reading __proto__ directly. Object.setPrototypeOf mutates that link after creation, which forces the engine to adjust internal shapes and can hurt performance on hot paths. Prefer setting the prototype at creation time with Object.create or class extends. For example, in a real app you might use getPrototypeOf in a test to assert that a factory returned an object wired to the expected base prototype, but avoid setPrototypeOf inside a game loop that spawns thousands of entities.',
