...
console.log(inner());`,
  ...
  },
  {
    ...
    answer: 'With var, all callbacks created inside a for loop share the same function-scoped i variable. By the time the setTimeout callbacks run, the loop has finished and i equals the final value, so every callback logs the same number. Using let gives each iteration its own block-scoped binding, which is why the fixed loop prints 0, 1, and 2. In a real app, attaching event listeners inside a loop with var can cause every button to act on the last index—switching to let or an IIFE that captures the current value fixes that bug.',
