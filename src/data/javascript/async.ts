...
console.log("sync");`,
  ...
  },
  {
    ...
    answer: 'A Promise represents a value that may not exist yet, wrapping async work so you can attach reactions with then and catch. It starts pending, moves to fulfilled when resolve runs with a value, or rejected when reject runs with a reason—and once settled it cannot change state again. That immutability makes Promise chains predictable and composable. For example, in a real app, fetch("/api/user") returns a pending Promise that becomes fulfilled with a Response or rejected on network failure, and your UI can show a spinner until one of those outcomes happens.',
...
p.catch(err => console.log("Rejected:", err));`,
  ...
  },
  {
    ...
    answer: 'Promise.then chains asynchronous steps through callbacks, while async/await lets you write the same flow with synchronous-looking code inside an async function. await pauses only that async function until the Promise settles, and errors bubble to try/catch instead of requiring .catch on every link. Under the hood, async functions still return Promises and compile to the same thenable machinery. In a real app, await fetchData(); await processData(data); is far easier to read and maintain than nested .then calls when loading a dashboard from several API endpoints.',
...
main();`,
  ...
  },
  {
    ...
    answer: 'Callback hell is deeply nested asynchronous callbacks that form a pyramid shape, making error handling and control flow hard to follow and test. Each step depends on the previous callback, so adding a new stage or handling errors at each level multiplies complexity quickly. Promises flatten the chain into sequential then calls, and async/await makes it read like synchronous code while preserving non-blocking behavior. In a real app, loading a user, then their permissions, then their dashboard widgets with nested callbacks is painful—rewriting as async/await with named functions or a small pipeline module keeps the same logic readable.',
