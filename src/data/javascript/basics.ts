...
let y = 10;`,
  ...
  },
  {
    ...
    answer: 'JavaScript has seven primitive types: string, number, bigint, boolean, undefined, null, and symbol. Primitives are immutable and compared by value, meaning two strings with the same characters are equal if their values match. Everything else—including arrays, functions, and plain objects—is an object type and is compared by reference.',
    code: `const types = [
  typeof "hello",   // string
  typeof 42,        // number
  typeof true,      // boolean
...
console.log(types.join(", "));`,
  ...
  },
  {
    ...
    answer: 'The == operator compares values after converting types if needed, which can produce surprising results like 0 == false being true. The === operator performs strict equality, checking both value and type without coercion, so 5 === "5" is false. Using === consistently avoids subtle bugs and is the standard in professional codebases and linters like ESLint.',
...
strictFn();`,
  ...
  },
  {
    ...
    answer: 'undefined means a variable has been declared but no value has been assigned yet, or a function returns nothing. null is an intentional assignment representing "no value" or an empty result that the developer chose explicitly. typeof undefined is "undefined", while typeof null is "object" due to a long-standing spec quirk.',
    code: `let a;
const b = null;
...
console.log(a === null); // false`,
  ...
  },
  {
    ...
    answer: 'JavaScript treats certain values as falsy in boolean contexts: false, 0, -0, 0n, empty string, null, undefined, and NaN. Every other value is truthy, including non-empty strings, arrays, objects, and even the strings "0" and "false".',
...
console.log(Boolean("0"));   // true`,
  ...
  },
  {
    ...
    answer: 'A function declaration defines a named function in a scope and is fully hoisted, so you can call it before its line in the file. A function expression assigns a function to a variable (often const) and only the variable name is hoisted—not the function body—so you cannot call it before that assignment runs. Arrow functions are always expressions.',
...
farewell(); // works now`,
  ...
  },
  {
    ...
    answer: 'An IIFE (Immediately Invoked Function Expression) is a function defined and executed in one expression, usually wrapped in parentheses. It creates a private scope so variables inside do not leak into the global namespace. Before ES modules, IIFEs were a common pattern for libraries and bundling code without naming collisions. })(); to keep internal helpers private while exposing only a public API.',
    code: `(function() {
  const secret = "hidden";
...
// console.log(secret); // ReferenceError`,
  ...
  },
  {
    ...
    answer: 'Primitives (numbers, strings, booleans, etc.) are passed by value, meaning the function receives a copy and reassigning the parameter does not affect the original. Objects, arrays, and functions are passed by sharing a reference—the copy points to the same underlying data, so mutating properties inside a function changes the original object. This distinction explains why incrementing a number parameter does not change the outer variable, but updating obj.name inside a helper does.',
    code: `let num = 5;
...
console.log(obj.x); // 99`,
  ...
  },
  {
    ...
    answer: 'NaN (Not-a-Number) is the value returned when a numeric operation fails, such as parsing a non-numeric string with Number(). Uniquely, NaN is not equal to itself, so NaN === NaN is false. The global isNaN() coerces its argument first, which can give false positives, while Number.isNaN() only returns true for actual NaN values.',
...
console.log(isNaN("hello"));        // true (coerced!)`,
  ...
  },
  {
    ...
    answer: 'typeof is a unary operator that returns a string naming the type of a value, such as "number", "string", or "boolean". It works well for primitives but has limitations: typeof null returns "object", and arrays also return "object". Functions return "function", which is a special case.',
