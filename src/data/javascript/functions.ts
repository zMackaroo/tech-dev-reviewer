...
console.log(counter()); // 2`,
  ...
  },
  {
    ...
    answer: 'Regular functions have their own dynamic this binding, an arguments object, can be used as constructors with new, and have a prototype property. Arrow functions inherit this lexically from the enclosing scope, have no arguments object, cannot be used with new, and are ideal for short callbacks. This difference causes the most bugs when passing methods as event handlers. In a real app, use a regular function for object methods that need this to refer to the object, and arrow functions inside class fields or React functional components where lexical this avoids manual bind calls.',
    code: `const obj = {
  name: "Test",
...
console.log(bound("?"));`,
  ...
  },
  {
    ...
    answer: 'Currying transforms a function that takes multiple arguments into a series of functions that each take one argument and return the next function until all arguments are collected. It enables partial application and creates specialized reusable functions from general ones. While related to partial application, currying strictly returns nested unary functions. For example, a curried add(a)(b)(c) pattern lets you define addTax = add(0.08) once and reuse it across a pricing module to apply the same tax rate to different amounts.',
...
console.log(doubled, evens, sum);`,
  ...
  },
  {
    ...
    answer: 'Debouncing wraps a function so it only runs after a quiet period with no new calls within a specified delay. Each new trigger resets the timer, canceling the previous scheduled execution. This prevents expensive work from running on every keystroke or resize event. In a real app, debounce an autocomplete search so the API is called only after the user stops typing for 300ms, reducing server load and avoiding race conditions from out-of-order responses.',
...
const search = debounce((q) => console.log("Search:", q), 300);`,
  ...
  },
  {
    ...
    answer: 'Throttling ensures a function runs at most once per time window, even if it is triggered continuously. Unlike debouncing, which waits for activity to stop, throttling fires on a steady cadence during ongoing events. This caps execution rate while still providing periodic updates. In a real app, throttle a scroll handler that updates a sticky header or loads more content—firing at most every 200ms keeps scrolling smooth without running layout calculations hundreds of times per second.',
