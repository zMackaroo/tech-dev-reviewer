...
// A, C print immediately; B later`,
  ...
  },
  {
    ...
    answer: 'A polyfill is runtime code that adds a missing API to the environment — for example, shimmed `Array.prototype.at` on an older browser that lacks it natively. Transpilation (via tools like Babel) converts newer syntax such as optional chaining or class fields into equivalent older syntax at build time. They solve different problems and are often used together: transpilation handles syntax the engine cannot parse, while polyfills handle APIs the engine can parse but does not implement. In a real app, Babel might convert `obj?.name` to safe property access while a polyfill adds `Promise.allSettled` for browsers that support the surrounding syntax but not that specific method.',
