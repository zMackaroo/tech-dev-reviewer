...
export const fundamentalsQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'TypeScript is a strongly typed programming language developed by Microsoft that builds on JavaScript by adding optional static types and modern language features. It is a superset of JavaScript, meaning any valid JavaScript file is also valid TypeScript, so you can adopt it incrementally without rewriting existing code. TypeScript code is compiled (transpiled) to plain JavaScript that runs in browsers, Node.js, or any JS runtime. In a real app, TypeScript helps teams catch bugs like passing a string where a number is expected before the code ever reaches production.',
...
console.log(message);`,
  },
  {
    ...
    answer: 'JavaScript is a dynamically typed language where variable types are determined at runtime and can change freely. TypeScript extends JavaScript with a static type system checked at compile time, catching type errors before the code runs. TypeScript does not execute directly in the browser—you compile it to JavaScript first using the TypeScript compiler (tsc) or a bundler like Vite or Webpack. For example, in plain JavaScript a function might silently accept any argument, while TypeScript flags mismatched types during development so you fix them in the editor instead of in production logs.',
...
const total: string = formatPrice(19.5, "USD");`,
  },
  {
    ...
    answer: 'Type inference is TypeScript\'s ability to automatically determine the type of a variable or expression without an explicit annotation. When you write const name = "Alice", the compiler infers string because the initial value is a string literal. Inference also works for function return types and complex expressions, reducing boilerplate while still providing type safety. In a real app, you often rely on inference for local variables and let the compiler infer return types for simple helpers, while explicitly annotating public API boundaries like exported functions and component props.',
...
export function Button({ label, onClick }: ButtonProps) {
  ...
  },
  {
    ...
    answer: 'Type erasure means all type information is removed during compilation and does not exist at runtime in the resulting JavaScript. Interfaces, type aliases, and generic type parameters are compile-time constructs only—you cannot inspect them with typeof or instanceof at runtime. This design keeps the runtime identical to hand-written JavaScript with zero type-related performance overhead. In a real app, this means you cannot use TypeScript types for runtime validation; you need separate checks like Zod or manual guards if you need to validate API responses at runtime.',
    code: `interface User {
  id: number;
  name: string;
}
...
// npx tsc --init`,
  },
  {
    ...
    answer: 'In React projects, TypeScript types component props, state, event handlers, and refs so UI code is self-documenting and less prone to runtime errors. You define props with interfaces or type aliases and destructure them in function components, while hooks like useState can be given explicit generic types when inference is insufficient. Tools like @types/react provide type definitions for React APIs. In a real app, typing a Modal component props as { isOpen: boolean; onClose: () => void; title: string } ensures every usage passes the required callbacks and prevents rendering bugs from missing props.',
...
export function Counter({ initialCount = 0, label }: CounterProps) {
  ...
  },
  {
    ...
    answer: 'Compile-time type checking happens when TypeScript analyzes your source code before it runs, reporting errors in the editor or during the build step. Runtime type checking happens while the program executes, using JavaScript mechanisms like typeof, instanceof, or validation libraries. TypeScript types are erased at compile time, so they provide zero runtime protection against bad data from external sources like APIs. For example, TypeScript ensures you treat a response as a User object in your code, but you still need runtime validation with a library like Zod to confirm the API actually returned that shape.',
    code: `interface User {
  id: number;
  name: string;
}
