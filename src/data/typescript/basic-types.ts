...
export const basicTypesQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'string, number, and boolean are TypeScript\'s primitive types for text, numeric values, and true/false flags respectively. Unlike JavaScript, TypeScript distinguishes these at compile time so you cannot assign incompatible values without an error. string covers any textual data, number includes integers and floats (there is no separate int or float type), and boolean is strictly true or false. In a real app, typing a form field as string and a quantity as number prevents accidentally concatenating a user-entered age with a string instead of performing arithmetic.',
...
if (typeof safe === "string") {
  ...
  },
  {
    ...
    answer: 'void represents the absence of a return value, most commonly used as the return type of functions that perform side effects but do not return anything meaningful. A function declared as returning void may still have a return statement with no value, or implicitly return undefined. void is different from undefined as a type—void is for function return types while undefined is a value type. In a real app, you annotate event handlers like onClick: () => void to signal they run for their side effects (updating state, logging) rather than producing a value callers should use.',
...
function handle(status: Status) {
  ...
  },
  {
    ...
    answer: 'null and undefined are both primitive types representing the absence of a value, but they carry different intent—undefined typically means not yet assigned, while null means intentionally empty. With strictNullChecks enabled (recommended), null and undefined are not assignable to other types unless you explicitly include them in a union like string | null. This prevents the common bug of calling methods on a value that might not exist. In a real app, typing a search result as User | null makes it clear the API may return no match, forcing you to handle the null case before accessing user properties.',
...
function findUser(id: string): { name: string } | null {
  ...
  },
  {
    ...
    answer: 'Literal types are specific values used as types rather than broad categories—like the literal "success" instead of the general string type. They are often combined with unions to create a fixed set of allowed values, similar to an enum but more lightweight. Literal types enable exhaustive checking and precise autocomplete in the editor. For example, typing a button variant as "primary" | "secondary" | "danger" ensures only valid design-system options are passed, catching typos like "primry" at compile time.',
...
let count: number = answer; // OK — 42 is assignable to number`,
  },
  {
    ...
    answer: 'Enums declare a set of named constants, either as numeric enums (auto-incrementing numbers) or string enums (explicit string values). They give readable names to magic values and can improve autocomplete when a fixed set of options is shared across a codebase. However, enums generate runtime JavaScript code unlike most type-only constructs, and many teams prefer const objects with as const or union types instead. In a real app, a string enum like enum Role { Admin = "ADMIN", User = "USER" } can represent database role values, though a union type "ADMIN" | "USER" achieves the same safety with zero runtime overhead.',
    code: `enum Status {
  Pending = "PENDING",
  Active = "ACTIVE",
  Archived = "ARCHIVED",
...
interface Service {
  ...
  },
  {
    ...
    answer: 'TypeScript offers two equivalent syntaxes for array types: Type[] (shorthand) and Array<Type> (generic syntax). Arrays are homogeneous by default—number[] means every element must be a number. For arrays that may contain multiple types, use a union like (string | number)[]. Readonly arrays prevent mutation with readonly string[] or ReadonlyArray<string>. In a real app, typing a tags prop as string[] ensures components receive an array of strings, while readonly Product[] signals that a list fetched from an API should not be mutated in place.',
