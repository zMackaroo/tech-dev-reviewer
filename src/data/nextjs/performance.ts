...
export default function Dashboard() {
  ...
  },
  {
    ...
    answer: 'Next.js automatically splits code per route in the App Router so each page ships only the JavaScript it needs, and Server Components further reduce client bundle size by keeping data-fetching logic on the server. You can analyze bundle composition with @next/bundle-analyzer, tree-shake unused exports, and mark heavy dependencies as server-only to prevent them from leaking into client bundles. Configuring modularizeImports or optimizePackageImports in next.config.ts lets Next.js import only the submodules you use from large libraries like lodash or icon packs. In a real app, moving a markdown parser to a Server Component and using optimizePackageImports for lucide-react can shave hundreds of kilobytes off the client bundle.',
    code: `// next.config.ts
