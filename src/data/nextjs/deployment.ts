...
export default nextConfig`,
  },
  {
    ...
    answer: 'Next.js loads variables from .env, .env.local, .env.production, and environment-specific files into process.env at build time. Variables prefixed with NEXT_PUBLIC_ are inlined into the client JavaScript bundle, making them visible in the browser, so they should never contain secrets like API keys or database passwords. Server-only variables without the prefix are available in Server Components, Route Handlers, and Server Actions but are stripped from client bundles.',
    code: `// .env.local
...
export default function Home() {
  ...
  },
  {
    ...
    answer: 'assetPrefix prepends a URL to static assets like JavaScript, CSS, and images served from /_next/static/, but does not change your application\'s route paths the way basePath does. It is commonly used when assets are hosted on a CDN separate from the application server, so browsers fetch JS chunks from cdn.example.com while HTML pages come from app.example.com. basePath affects routing; assetPrefix affects only where static files are loaded from.',
    code: `// next.config.ts
import type { NextConfig } from 'next'
