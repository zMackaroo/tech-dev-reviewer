...
// npm run build && npm run start`,
  },
  {
    ...
    answer: 'React Server Components (RSC) are components that render on the server and send HTML to the client without shipping their JavaScript bundle. In the App Router, components are Server Components by default—they can be async, query databases directly, and access server-only modules. Client Components marked with "use client" handle interactivity like useState and onClick handlers.',
...
export function AddToCart({ productId }: { productId: string }) {
  ...
  },
  {
    ...
    answer: 'Next.js is an open-source framework originally created by Vercel and can be deployed anywhere that runs Node.js, Docker, or static hosting. Vercel is the company that maintains Next.js and offers a hosting platform with first-class support for its features like ISR, edge functions, and preview deployments. Using Vercel is optional—many teams run Next.js on AWS, Railway, or self-managed infrastructure.',
...
// Deploy targets: Vercel, AWS, Docker, Node server, etc.`,
  },
  {
    ...
    answer: 'Hybrid rendering means different routes in the same application can use different rendering strategies—static, server-rendered, or client-rendered—chosen per page based on its data and freshness needs. Next.js decides at build time and request time how each segment should render, and you can opt in with exports like dynamic, revalidate, or fetch cache options. This avoids forcing one global approach on every page.',
